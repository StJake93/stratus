...
<div class="wbox">
  <div class="whead"><span class="wtag">Interactive</span><h4>CIDR &amp; subnet calculator</h4></div>
  <div class="two">
    <Range label="VPC prefix (AWS allows /16 – /28)" bind:value={vpc} min={16} max={28} format={(v) => `10.0.0.0/${v}`} />
    <Range label="Subnet prefix" bind:value={sub} min={16} max={28} format={(v) => `/${v}`} />
  </div>

  <div class="bits" aria-label="Subnet mask bits">
    {#each maskBits as net, i}
      <span class:net class:vpcpart={i < vpc} class:gap={i % 8 === 7}></span>
    {/each}
  </div>
  <div class="legend"><span><i class="a"></i> VPC network bits ({vpc})</span><span><i class="b"></i> Subnet bits ({sub - vpc})</span><span><i class="c"></i> Host bits ({32 - sub})</span></div>
  ...
  <div class="blocks">
    {#each Array(shown) as _, i}
      <span class="blk" style:animation-delay="{i * 8}ms" title={i < cidrs.length ? cidrs[i] : ''}></span>
    {/each}
    {#if subnets > shown}<span class="more">+{fmt(subnets - shown)} more</span>{/if}
  </div>
  <p class="list">{cidrs.join(' · ')}{subnets > cidrs.length ? ' · …' : ''}</p>
  <p class="faint note">AWS reserves 5 addresses in every subnet: network address, VPC router, DNS, one for future use, and broadcast.</p>
</div>
...
<style>
  ...
  .blk {
    ...
    height: 22px;
    border-radius: 5px;
    background: linear-gradient(135deg, rgba(34, 211, 238, 0.5), rgba(124, 92, 255, 0.5));
