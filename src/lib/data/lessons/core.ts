    ...
    steps: [
      ...
      },
      {
        ...
        blocks: [
          ...
          tabs(
            ...
            ['Serverless / FaaS', [text('**Functions as a Service**: upload a function and it runs on events. Scales to zero, billed per millisecond.', '', '- AWS: **Lambda**', '- Azure: Functions', '- GCP: Cloud Run functions'), info('“Serverless” doesn’t mean no servers. It means *you* never see or manage them.')]],
            ['SaaS', [text('**Software as a Service**: finished applications such as email, CRM and observability tools. You just configure and use them.')]]
          ),
      ...
      },
      {
        ...
        blocks: [
          ...
          quiz('cloud-101', [
            q('A team runs a web app on EC2 instances. Who is responsible for patching the guest operating system?', ['AWS', 'The customer', 'Shared equally', 'Nobody, because EC2 patches itself'], 1, 'EC2 is IaaS. AWS runs the hypervisor and hardware; the OS on the instance is yours to patch (tools like Systems Manager Patch Manager help).'),
            ...
            q('Which is the best description of elasticity?', ['Paying up front for 3 years', 'Automatically matching capacity to demand', 'Running in multiple regions', 'Using open-source software'], 1, 'Elasticity means adding and removing capacity as demand changes, so you pay only for what you use.')
          ]),
  ...
  // ------------------------------------------------------------------
  {
    ...
    steps: [
      ...
      },
      {
        ...
        blocks: [
          text('Amazon CTO Werner Vogels put it simply: “Everything fails, all the time.” Cloud architecture assumes components *will* fail and plans around it.'),
          table(
          ...
          quiz('regions-azs', [
            q('Your app runs on two EC2 instances, both in `ap-southeast-2a`. What failure takes the whole app down?', ['A single instance crash', 'An outage of AZ a', 'An outage of another region', 'None, because two instances are enough'], 1, 'Both instances share one AZ, so an AZ-level event takes both out. Spread instances across AZs.'),
        ...
        blocks: [
          tabs(
            [
              'Declarative (what)',
              [
                text('You describe the **desired end state**. The tool figures out the steps to get there, and on the next run applies only the difference.'),
                code('hcl', `resource "aws_s3_bucket" "logs" {
  bucket = "acme-logs"
}`, 'main.tf', 'Terraform: “there should be a bucket called acme-logs”. Run it twice and nothing changes the second time.')
              ]
            ],
            [
              'Imperative (how)',
    ...
    steps: [
      {
        ...
        blocks: [
          text(
            'A **container** packages your application together with its runtime, libraries and config into a single, portable unit. It runs the same on your laptop, in CI and in production, which solves “works on my machine”.',
            '',
